package study;

import org.assertj.core.api.Assertions;
import org.junit.jupiter.api.Test;

import java.util.HashMap;
import java.util.Map;

public class StringCalculatorTest {

    //사용자가 입력한 문자열 값에 따라 사칙연산을 수행할 수 있는 계산기를 구현해야 한다.
    //문자열 계산기는 사칙연산의 계산 우선순위가 아닌 입력 값에 따라 계산 순서가 결정된다. 즉, 수학에서는 곱셈, 나눗셈이 덧셈, 뺄셈 보다 먼저 계산해야 하지만 이를 무시한다.
    //예를 들어 "2 + 3 * 4 / 2"와 같은 문자열을 입력할 경우 2 + 3 * 4 / 2 실행 결과인 10을 출력해야 한다.

    private final Calculator calculator = new Calculator();

    @Test
    void calculateTest() {
        String value = "2 + 3 * 4 / 2";
        int result = calculate(value);

        Assertions.assertThat(result).isEqualTo(10);
    }

    int calculate(String inputStr) {
        String[] values = inputStr.split(" ");
        int result = Integer.parseInt(values[0]);
        for (int i = 1; i < values.length - 1; i+=2) {
            String op = values[i];
            int target = Integer.parseInt(values[i+1]);
            result = calculator.calculate(op, result, target);
        }
        return result;
    }

    interface Command {
        int calculate(int a, int b);
    }

    static class Calculator {
        private Map<String, Command> commandMap;

        public Calculator() {
            this.commandMap = new HashMap<>();
            this.commandMap.put("+", (a, b) -> a + b);
            this.commandMap.put("-", (a, b) -> a - b);
            this.commandMap.put("/", (a, b) -> a / b);
            this.commandMap.put("*", (a, b) -> a * b);
        }

        public void addCommand(String op, Command command) {
            commandMap.put(op, command);
        }

        public int calculate(String op, int a, int b) {
            Command command = commandMap.get(op);
            if(command == null) {
                throw new IllegalArgumentException();
            }
            return command.calculate(a, b);
        }
    }

}